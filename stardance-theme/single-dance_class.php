    ...
    <section class="sd-section sd-class-times" id="class-times">
        <div class="sd-container">
            ...
            <div class="sd-schedule-page__grid sd-class-times__schedule-grid">
                ...
                ?>
                <div class="sd-class-times__schedule-row sd-class-times__schedule-row--top">
                    <?php foreach ( $sd_top_row as $index => $row ) :
                        $day_label   = $row[0];
                        $time_key    = $row[1];
                        $closed_key  = $row[2];
                        $fallback    = $row[3];
                ...
                <div class="sd-class-times__schedule-row sd-class-times__schedule-row--bottom">
                    <?php foreach ( $sd_bottom_row as $index => $row ) :
                        $day_label   = $row[0];
                        $time_key    = $row[1];
                        $closed_key  = $row[2];
                        $fallback    = $row[3];
                        $delay       = max( 1, min( 7, $index + 5 ) );
                        $acf_time    = $sd_has_acf ? trim( (string) get_field( $time_key, $sd_class_id ) ) : '';
                        $acf_closed  = $sd_has_acf ? (bool) get_field( $closed_key, $sd_class_id ) : false;
                        $is_closed   = $acf_closed || ( '' === $acf_time && 'Closed' === $fallback );
                        $time_text   = $is_closed ? 'Closed' : ( '' !== $acf_time ? $acf_time : $fallback );
...
<?php if ( 'kids-program' === $sd_class_slug ) : ?>
                <div class="sd-class-video__player" data-video-player>
                    <video
                    ...
                    <button type="button" class="sd-video__play-btn sd-class-video__play" aria-label="<?php echo esc_attr__( 'Play video', 'stardance' ); ?>">
                        <img src="https://stardance.com.cy/wp-content/uploads/2026/03/play-button-blue.svg" alt="" width="80" height="80" decoding="async">
                    </button>
                    <img
